#!/usr/bin/env python3
"""
Publish pipeline artifacts from data/ to docs/ for GitHub Pages.
Stdlib only. Keeps branding/config in one place.
"""
...
SITE_META = {
    ...
    "course": "INF 6420 Final",
    "author": "Katie Rock",
    "email": "fn9575@wayne.edu",
    "repo_url": "https://github.com/katierock01/court-docs-pipeline",
}

SOURCE_DIR = Path("data")
DOCS_ROOT = Path("docs")
DEST_DIR = DOCS_ROOT / "data"
...
def validate_inputs() -> bool:
    missing = [fn for fn in REQUIRED_ARTIFACTS if not (SOURCE_DIR / fn).exists()]
    if missing:
        print(f"[ERROR] Missing required artifacts in {SOURCE_DIR}: {missing}")
        return False
    return True
...
def sync_data() -> int:
    copied = 0
    for filename in REQUIRED_ARTIFACTS + OPTIONAL_ARTIFACTS:
        ...
        if src.exists():
            ...
            copied += 1
        elif filename in OPTIONAL_ARTIFACTS:
...
def sync_assets() -> None:
    ...
    if css_src.exists():
        ...
        print("   [WARN] css/ not found locally; created empty docs/css/")
...
<head>
    ...
    <title>{meta['title']} | Pipeline</title>
...
<body>
    <div class="navbar">
      <div>
        <h1>{meta['title']}</h1>
        <div class="sub-brand">{meta['course']} — {meta['author']} — {meta['email']}</div>
      </div>
      <div>
        <a href="{meta['repo_url']}" target="_blank">View Repository ↗</a>
      </div>
    </div>
...
"""
    ...
    print("   [OK] Generated branded index.html with repo link")


def finalize_pages() -> None:
...
def main() -> int:
    ...
    print(f"\n[DONE] {copied} files published to {DEST_DIR}.")
